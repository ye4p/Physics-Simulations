...
{
    Molecule m = Molecule(Vector(WINDOW_WIDTH/2, WINDOW_HEIGHT/2), Vector(10, 0), 10, 10);
    vec.push_back(m);
...
SDL_AppResult SDL_AppIterate(void *appstate)
{
    ...
    for (const Molecule &m : vec) {
        SDL_RenderPoint(renderer, (float) m.position.x, (float) m.position.y);
    }
